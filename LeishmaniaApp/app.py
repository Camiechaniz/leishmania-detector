...
# ==================== RUTAS GLOBALES ====================
BASE_DIR = Path("/content/drive/Shareddrives/TESIS")
MODEL_DIR = BASE_DIR / "YOLO_MODELOS"
MACROFAGOS_PATH = MODEL_DIR / "best_macrofagos.pt"
PARASITOS_PATH = MODEL_DIR / "best_parasitos.pt"
OUTPUT_DIR = Path("/content/salidas") / time.strftime("%Y%m%d_%H%M%S")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# ARCHIVOS GLOBALES PARA HISTORIAL
HISTORIAL_CSV = Path("/content/HISTORIAL_TODOS_DATASETS.csv")
GRAFICO_DATA_GLOBAL = Path("/content/grafico_data_global.json")
...
""")
    ...
    st.image(
        "/content/assets/FLUJOGRAMA PAGINA WEB.jpg",
...
with tab_analizar:
    ...
    if st.button("Descargar ZIP del dataset actual", type="primary"):
        ...
        zip_path = Path("/content") / f"{dataset_id}_COMPLETO.zip"
