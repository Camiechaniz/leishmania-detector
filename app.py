...
# ==================== CONFIGURACIÓN CRÍTICA PARA RENDER (AL PRINCIPIO!!) ====================
os.environ["CUDA_VISIBLE_DEVICES"] = ""  # Forzar CPU
os.environ["YOLO_CONFIG_DIR"] = "/tmp/yolo_config"
os.environ["YOLO_CACHE_DIR"] = "/tmp/yolo_cache"
os.makedirs("/tmp/yolo_config", exist_ok=True)
os.makedirs("/tmp/yolo_cache", exist_ok=True)

# ==================== PARÁMETROS GLOBALES FIJOS (NO LOS PISAR MÁS ABAJO) ====================
YOLO_SIZE = 640                  # ← CRUCIAL: 640, no 1024
CONF_MACROFAGOS = 0.60           # ← más bajo = detecta más (bueno para menos imágenes)
CONF_PARASITOS = 0.40

st.set_page_config(page_title="Detector Leishmania", page_icon="Microbe", layout="wide")
...
# ==================== CARGAR MODELOS (con spinner y forzado CPU) ====================
...
def get_model_macrofagos():
    ...
    if _model_macrofagos is None:
        with st.spinner("Cargando modelo de macrófagos... (solo la primera vez)"):
            _model_macrofagos = YOLO(MACROFAGOS_PATH)
            _model_macrofagos.to('cpu')
    return _model_macrofagos

def get_model_parasitos():
    global _model_parasitos
    if _model_parasitos is None:
        with st.spinner("Cargando modelo de parásitos... (solo la primera vez)"):
            _model_parasitos = YOLO(PARASITOS_PATH)
            _model_parasitos.to('cpu')
    return _model_parasitos
# ==================== UTILIDADES ====================
def imread_robusto(data):
    """Carga robusta de imágenes desde bytes o path. Compatible con Streamlit y evita cuelgues de PIL."""
    try:
        ...
        # Si no era bytes, intento como path
...
# ==================== DETECCIÓN PRINCIPAL ====================
def detect_on_S_with_border(S_gray, edge_mask, conf_macro=CONF_MACROFAGOS, conf_para=CONF_PARASITOS):
    H, W = S_gray.shape[:2]
    S_rgb = cv2.cvtColor(S_gray, cv2.COLOR_GRAY2BGR)
    S_lb, ratio, pad = letterbox_any(S_rgb, new_shape=YOLO_SIZE)

    macro_res = get_model_macrofagos().predict(
        source=S_lb, imgsz=YOLO_SIZE, conf=conf_macro, verbose=False,
        device="cpu", batch=1, workers=0, stream=False
    )[0]

    para_res = get_model_parasitos().predict(
        source=S_lb, imgsz=YOLO_SIZE, conf=conf_para, verbose=False,
        device="cpu", batch=1, workers=0, stream=False
    )[0]
...
if uploaded_files:
    ...
    if st.session_state.image_source == "Dispositivo móvil":
        ...
        if st.session_state.use_global_crop_cell:
            st.session_state.global_crop_w = st.slider(
                ...
                100, W0,
                min(st.session_state.global_crop_w, W0),
                key="global_crop_w"
            )
            ...
            st.session_state.global_crop_h = st.slider(
                ...
                100, H0,
                min(st.session_state.global_crop_h, H0),
                key="global_crop_h"
            )
    ...
    # ----- PROCESAR -----
    if st.button("Procesar todas las imágenes", type="primary", use_container_width=True, key="procesar_final"):
            
        # =============== SEGURIDAD PARA RENDER ===============
        if not st.session_state.uploaded_images:
            st.error("⚠️ Primero tenés que subir al menos una imagen")
            st.stop()

        if len(st.session_state.uploaded_images) > 25:
            st.error("🚨 En Render gratuito el máximo son **25 imágenes** por procesamiento.\n"
                    "Subí menos imágenes o dividí el dataset.\n"
                    "Si necesitás procesar más, usá un plan pago o local.")
            st.stop()
        ...
        for idx, (name, data) in enumerate(st.session_state.uploaded_images.items()):
            ...
            status_text.info(f"Procesando {idx + 1}/{len(st.session_state.uploaded_images)} → {name}")

            img_bgr = imread_robusto(data)
            mode = st.session_state.image_modes.get(name, "Dispositivo móvil")

            # Recorte o fondo circular
            if mode == "Celular" and st.session_state.use_global_crop_cell and st.session_state.use_crop.get(name, False):
                p = st.session_state.crop_params[name]
            ...
            # Detección (ya tenés YOLO_SIZE=640 y device='cpu' en la función)
            ...
            imagen_preview.image(cv2.cvtColor(det_vis, cv2.COLOR_BGR2RGB), 
                                caption=f"Resultado → {name}", use_container_width=True)

            # Guardar archivos
            base = Path(name).stem
            cv2.imwrite(str(out_dir / f"{base}_det.jpg"), det_vis)
            pd.DataFrame(det_rows, columns=["x1","y1","x2","y2","conf","invalid","tipo"]).to_csv(
                out_dir / f"{base}_det.csv", index=False)

            # Guardar fila de resumen
            tasa = round(para_v / ok_m * 200, 2) if ok_m > 0 else 0
            st.session_state.image_data.append({
                "dataset_id": st.session_state.dataset_id,
                "image_name": name,
                "ok_macrofagos": ok_m,
                "parasites_validos": para_v,
                "total_macrofagos": total_m,
                "macrofagos_invalidos": touch_m,
                "parasitos_totales": para_t,
                "tasa_infeccion": tasa
            })

            contador_nuevas += 1
            prog.progress((idx + 1) / len(st.session_state.uploaded_images))
            gc.collect()

        # =============== LIMPIAR Y FINALIZAR ===============
        prog.empty()
        status_text.empty()
        imagen_preview.empty()

        if contador_nuevas == 0:
            st.warning("Todas las imágenes ya estaban procesadas")
        else:
            st.success(f"¡{contador_nuevas} imágenes procesadas correctamente! 🎉")
            st.balloons()

        st.write(f"**TOTAL ACUMULADO:** {st.session_state.total_ok_macrofagos} macrófagos válidos | "
                f"{st.session_state.total_parasites} parásitos válidos")

        if st.session_state.total_ok_macrofagos >= 200:
            st.success("¡Ya tenés 200+ macrófagos válidos! Podés agregar el punto al gráfico")
        ...
        try:
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as z:
                for archivo in dataset_path.rglob("*"):
                    if archivo.is_file():
                        z.write(archivo, archivo.relative_to(dataset_path.parent.parent))
